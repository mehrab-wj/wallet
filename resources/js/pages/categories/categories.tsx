            ...
            return (
                <Card>
                    <CardContent className="flex flex-col items-center justify-center py-12">
                        ...
                        <CardTitle className="mb-2 text-center">No {type} categories yet</CardTitle>
                            ...
                            <CardHeader>
                                <div className="flex items-start justify-between">
                                    <div>
                                        ...
                                        <CardDescription>{children.length > 0 && `${children.length} subcategories`}</CardDescription>
                                    </div>
                                        ...
                                        <Dialog>
                                            ...
                                            <DialogContent>
                                                ...
                                                <DialogDescription>
                                                    ...
                                                    {children.length > 0 && ` This will remove ${children.length} subcategories as well.`} This action
                                                    cannot be undone.
                                                </DialogDescription>
                            ...
                            {children.length > 0 && (
                                <CardContent>
                                    <div className="space-y-2">
                                        {children.map((child) => (
                                            <div key={child.id} className="flex items-center justify-between rounded-md bg-muted/50 p-2">
                                                <span className="text-sm">{child.name}</span>
                                                <div className="flex gap-1">
                                                    <Button variant="ghost" size="icon" onClick={() => handleEdit(child)} className="h-6 w-6">
        ...
        >
            ...
            <div className="space-y-6 p-4">
                ...
                <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as CategoryType)}>
                    <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
                        ...
                        <Drawer open={isCreateDrawerOpen} onOpenChange={setIsCreateDrawerOpen}>
                            ...
                            <DrawerContent>
                                <Form method="post" action={route('categories.store')} onSuccess={resetCreateDrawer} resetOnSuccess>
                                    {({ processing, errors }) => (
                                        <>
                                            <DrawerHeader>
                                                <DrawerTitle>Create New {activeTab === 'expense' ? 'Expense' : 'Income'} Category</DrawerTitle>
                                                <DrawerDescription>Add a new {activeTab} category to organize your transactions</DrawerDescription>
                                            </DrawerHeader>
                    ...
                    <TabsContent value="expense">{renderCategoriesList('expense')}</TabsContent>

                    <TabsContent value="income">{renderCategoriesList('income')}</TabsContent>
                </Tabs>
                ...
                <Drawer open={isEditDrawerOpen} onOpenChange={setIsEditDrawerOpen}>
                    <DrawerContent>
                        {editingCategory && (
                            <Form method="put" action={route('categories.update', editingCategory.id)} onSuccess={resetEditDrawer}>
                                {({ processing, errors }) => (
                                    <>
                                        <DrawerHeader>
                                            <DrawerTitle>Edit Category</DrawerTitle>
                                            <DrawerDescription>Update your category information</DrawerDescription>
                                        </DrawerHeader>
